...
import com.naminhyeok.fantazzk.teambuilding.TeamBuildingExceptionHandler
...
import com.naminhyeok.fantazzk.teambuilding.template.TemplateLookUpService
...
class RoomApiControllerTest {
    ...
    private val mockMvc: MockMvc =
        MockMvcBuilders
            ...
            .setControllerAdvice(TeamBuildingExceptionHandler())
            .build()
    ...
    inner class `방 생성` {
        ...
        fun `유효한 요청으로 방을 생성하면 201을 반환한다`() {
            ...
            every { templateLookUpService.getPlayers(template.templateId) } returns emptyList()
            every { roomCreateService.create(any(), any(), "호스트") } returns room
            every { roomLookUpService.getTeamLeaders(room.roomId) } returns emptyList()
        ...
        fun `존재하지 않는 템플릿으로 생성하면 404를 반환한다`() {
            every { templateLookUpService.get(any()) } throws com.naminhyeok.fantazzk.teambuilding.exception.TemplateNotFoundException()
